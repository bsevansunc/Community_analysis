...
summary.outs(t.abund)[[1]]

# Beta estimates:

summary.outs(t.abund)[[2]]  
...
write.csv(summary.outs(t.abund)[[1]],
          'output/table_outs/abundance2/total/aic_table.csv',
          row.names = F)
write.csv(summary.outs(t.abund)[[2]],
          'output/table_outs/abundance2/total/beta_estimates_table.csv',
          row.names = F)
write.csv(HL.table(t.abund),
          'output/table_outs/abundance2/total/HLtable.csv',
          row.names = F)
...
out.fun = function(i){
  ...
  write.csv(summary.outs(pc.abund[,i])[[1]],
            paste(file.name,'/aic_table.csv',sep =''),
            row.names = F)
  write.csv(summary.outs(pc.abund[,i])[[2]],
            paste(file.name,'/beta_estimates_table.csv',sep =''),
            row.names = F)
  write.csv(HL.table(pc.abund[,i]),
