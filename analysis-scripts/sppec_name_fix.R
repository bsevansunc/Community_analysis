...
fix.sp.names = function(df){
  ...
  df$spec = gsub('cedw','cewa',df$spec) # Cedar waxwing
  ...
  df$spec = gsub('unid-x','unbi',df$spec) # UNBI
  df$spec = gsub('unid (woodpecker knocking)','unbi',df$spec) # UNBI
  df$spec = gsub('unid gull','unbi.w',df$spec) # UNBI
